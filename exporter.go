...
var statusCodeRangeRe = regexp.MustCompile("[0-9]xx")
...
func parseStatusCodes(codes string) ([]int, error) {
	...
	for _, code := range splitCodes {
		...
		if statusCodeRangeRe.MatchString(code) {
		...
		if err != nil {
			return nil, fmt.Errorf("can not convert status code %q to a number", code)
		}
